...
import { show, hide, isVisible, moveSelection, confirmSelection } from './dropdown'
import { getCurrentText, getCaretIndex, getCaretRect, inject } from './injector'
...
async function init() {
  ...
  if (!state.allowedDomains.some(d => location.hostname === d || location.hostname.endsWith('.' + d))) return
...
function attachListener(detected: DetectedElement) {
  ...
  detected.el.addEventListener('keyup', (e) => {
    ...
    if (['Shift', 'Control', 'Alt', 'Meta', 'CapsLock', 'Tab',
         ...
         'F5', 'F6', 'F7', 'F8', 'F9', 'F10', 'F11', 'F12'].includes(key)) return
  ...
  detected.el.addEventListener('keydown', (e) => handleKeydown(e as KeyboardEvent, detected))
...
function handleKeydown(e: KeyboardEvent, _detected: DetectedElement) {
  ...
  else if (e.key === 'Enter') { e.preventDefault(); e.stopPropagation(); confirmSelection() }
