...
export interface DetectedElement {
  el: TargetElement
  type: 'slate' | 'react-textarea' | 'comfyui' | 'midjourney' | 'contenteditable' | 'generic'
}
...
function classify(el: Element): DetectedElement['type'] {
  ...
  if (el.tagName === 'TEXTAREA' && (el as HTMLElement).id === 'desktop_input_bar') return 'midjourney'
...
export function startDetector(
  ...
  function check(root: Element | Document) {
    const candidates = root instanceof Document
      ? document.querySelectorAll<Element>(SELECTORS.join(','))
      : (root.matches?.(SELECTORS.join(',')) ? [root] : root.querySelectorAll<Element>(SELECTORS.join(',')))
