...
export function getCurrentText(detected: DetectedElement): string {
  if (detected.type === "slate") {
    const spans = (detected.el as HTMLElement).querySelectorAll<HTMLElement>(
      '[data-slate-string="true"]',
    );
    // Fall back to textContent if Slate string spans are absent (newer Slate versions)
    const slateText = Array.from(spans)
      .map((s) => s.textContent ?? "")
      .join("");
...
export function getCaretIndex(detected: DetectedElement): number {
  ...
  return (
    (detected.el as HTMLInputElement | HTMLTextAreaElement).selectionStart ?? 0
  );
}
...
export function inject(
  ...
  if (detected.type === "slate") {
    ...
    injectContentEditable(
      detected.el as HTMLElement,
      atIndex,
      caretIndex,
      value,
    );
  } else if (
    detected.type === "react-textarea" ||
    detected.type === "comfyui" ||
    detected.type === "midjourney"
  ) {
    injectReactTextarea(
      detected.el as HTMLTextAreaElement,
      atIndex,
      caretIndex,
      value,
    );
    ...
    injectGeneric(
      detected.el as HTMLInputElement | HTMLTextAreaElement,
      atIndex,
      caretIndex,
      value,
    );
