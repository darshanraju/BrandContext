...
interface Props { domains: string[] }
  ...
  return (
    <div className="allowed-sites">
      ...
      <p className="allowed-sites-desc">
        The @ autocomplete activates only on these domains. Add a hostname like <code>dream.ai</code> — no protocol needed.
      </p>
      {domains.map(d => (
        <div key={d} className="domain-row">
          <span className="domain-text">{d}</span>
          <button className="btn-icon" onClick={() => removeDomain(d)}><TrashIcon size={14} /></button>
        </div>
      ))}
      <div className="add-domain-row">
        <input className="domain-input" value={input} placeholder="e.g. chatgpt.com"
