...
import { PlusIcon, PencilIcon, DownloadIcon, UploadIcon } from '../../shared/icons'

interface Props { brand: Brand | null; allBrands: Brand[] }
...
export function ShortcutTable({ brand, allBrands }: Props) {
  ...
  useEffect(() => { if (addingRow) addKeyRef.current?.focus() }, [addingRow])
  useEffect(() => { if (renamingBrand) renameRef.current?.focus() }, [renamingBrand])
  ...
  function cancelAdd() { setNewKey(''); setNewVal(''); setAddingRow(false) }

  async function saveBrandName() {
    const name = brandName.trim()
    if (!name || !brand) { setRenamingBrand(false); setBrandName(brand?.name ?? ''); return }
    const { brands: latest } = await getState()
    await updateBrands(latest.map(b => b.id === brand.id ? { ...b, name } : b))
    ...
    try {
      ...
      const brands: Brand[] = parsed.brands.map((b: Brand) => ({
        ...
        shortcuts: Array.isArray(b.shortcuts)
          ? b.shortcuts
              .filter((s: { key?: unknown; value?: unknown }) => s.key && s.value)
              .map((s: { key: string; value: string }) => ({ key: String(s.key), value: String(s.value) }))
          : [],
      }))
      if (!brands.length) { setImportError('No brands found in file'); return }
  ...
  async function onDrop(dropIndex: number) {
    ...
    await updateBrands(latest.map(b => b.id === brand.id ? { ...b, shortcuts } : b))
  }
    ...
    <>
      <div className="main-header">
        <div style={{ display: 'flex', alignItems: 'center' }}>
          {renamingBrand ? (
            <input ref={renameRef} className="table-edit-input val" style={{ fontSize: 16, fontWeight: 600 }}
              value={brandName} onChange={e => setBrandName(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter') saveBrandName(); if (e.key === 'Escape') { setRenamingBrand(false); setBrandName(brand.name) } }}
              onBlur={saveBrandName} />
          ) : (
            <>
              <span className="main-title">{brand.name}</span>
              <span className="shortcut-count">{brand.shortcuts.length} shortcuts</span>
            </>
          )}
        </div>
        <div className="header-actions">
          ...
          <button className="btn-sm" onClick={handleImportClick} title="Import all brands from JSON">
            <UploadIcon size={13} />Import
          </button>
          <button className="btn-sm" onClick={handleExport} title="Export all brands as JSON">
            <DownloadIcon size={13} />Export
          </button>
          <button className="btn-sm primary" onClick={() => setAddingRow(true)}>
            <PlusIcon size={13} />Add shortcut
          </button>
        </div>
      </div>
      ...
      {importError && (
        <div style={{
          margin: '8px 20px', padding: '7px 12px', borderRadius: 'var(--r-md)',
        ...
        {addingRow && (
          <div className="table-row editing" style={{ gridTemplateColumns: '1fr auto' }}>
            <div className="cell" style={{ gap: 6 }}>
              <input ref={addKeyRef} className="table-edit-input key" value={newKey} placeholder="key"
                onChange={e => setNewKey(e.target.value)}
                onKeyDown={e => { if (e.key === 'Enter') addShortcut(); if (e.key === 'Escape') cancelAdd() }} />
              <input className="table-edit-input val" value={newVal} placeholder="value"
                onChange={e => setNewVal(e.target.value)}
                onKeyDown={e => { if (e.key === 'Enter') addShortcut(); if (e.key === 'Escape') cancelAdd() }} />
            </div>
            <div className="cell cell-actions">
              <button className="btn-save" onClick={addShortcut}>Add</button>
