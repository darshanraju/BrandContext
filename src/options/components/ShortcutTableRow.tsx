...
export function ShortcutTableRow({
  shortcut,
  brand,
  isDragOver,
  onDragStart,
  ...
  async function save() {
    ...
    const updated = latest.map((b) =>
      b.id === brand.id
        ? {
            ...b,
            shortcuts: b.shortcuts.map((s) =>
              s.key === shortcut.key ? { key: k, value: v } : s,
            ),
          }
        : b,
    );
    ...
    await updateBrands(
      latest.map((b) =>
        b.id === brand.id
          ? {
              ...b,
              shortcuts: b.shortcuts.filter((s) => s.key !== shortcut.key),
            }
          : b,
      ),
    );
    ...
    >
      <div className="cell cell-key">
        <span className="drag-handle">
          <DragHandleIcon size={14} color="var(--border)" />
        </span>
        <span className="key-chip">@{shortcut.key}</span>
      </div>
      <div className="cell cell-val">
        {isHexColor(shortcut.value) && (
          <div
            className="opt-color-swatch"
            style={{ background: shortcut.value }}
          />
        )}
