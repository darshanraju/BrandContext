...
export async function getState(): Promise<StorageState> {
  const result = await chrome.storage.sync.get(['brands', 'activeBrandId', 'allowedDomains'])
  return {
    brands: (result['brands'] as Brand[] | undefined) ?? DEFAULTS.brands,
    activeBrandId: (result['activeBrandId'] as string | null | undefined) ?? DEFAULTS.activeBrandId,
    allowedDomains: (result['allowedDomains'] as string[] | undefined) ?? DEFAULTS.allowedDomains,
  }
}
